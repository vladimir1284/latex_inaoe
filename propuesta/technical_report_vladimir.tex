...
%	\thispagestyle{empty}
	...
	\begin{abstract}
		...
		in two directions, the computation of all the reducts and the computation of globally shortest reducts.
...
%  such that $V_a=V_c \cup V_d$. 
 ...
 \end{table}
  ...
  called decision tables. In table~\ref{tab_IS}, $c_1$ and $c_2$ are condition attributes.
...
\subsection{Discernibility Matrix and Discernibility Function}
 ...
 \end{table}
  ...
  conjunction ($\wedge$) boolean operators have their common meaning.
...
\subsection{Simplified Discernibility Matrix}
  ...
  system~\citep{Lazo01}. Table~\ref{tab_SDM} shows the simplified discernibility matrix for the discernibility
  matrix from table~\ref{tab_DM}.
  
     \begin{table}[htb]
		\caption{Simplified Discernibility Matrix Example.} \label{tab_SDM}
		\centering
 	\begin{tabular}{c|ccccccc}
 		$x \in U$ & 1 & 2 &  3 & 4 & 5 &  6 & 7\\
 		\hline
...
\section{Related Work}\label{relatedWork}
  ...
  In figure~\ref{fig_Tax} we propose a taxonomy of the reported algorithms for reduct computation. 
...
%  publication reporting the algorithm. 
  Some parallel approaches are implementations of existing sequential 
  algorithms. For those cases, we use a color association in figure~\ref{fig_Tax} such  that two related
  algorithms are written in the same color (different to black).
...
\subsection{Algorithm Finding a Single Reduct}
  ...
  attributes and adds, one at a time, the attribute having the highest significance. 
...
\subsection{Algorithms for all Reducts and all Typical Testors Computation}
  ...
  \cite{Sanchez07} proposed the CT\_EXT algorithm for computing all the
  ...
  \cite{WangP07} proposed a new algorithm for computing all the reducts (RGonCRS). Even though this algorithm 
  ...
  \item A second algorithm (SRGonCRS) is proposed for subdividing the dataset and the reducts are
  		incrementally found.
  \end{itemize}
...
\subsection{Research Objectives}\label{Goals} 
  ...
  in most datasets, and faster in some kind of datasets}. These algorithms will use some properties 
  ...
  shortest reducts, in information systems. The problem of finding shortest reducts has also 
...
%  		for computing globally shortest reducts.
  ...
  \item Developing a new algorithm for computing globally shortest reducts in information systems.
%  
...
%  		be explained using the same theoretical basis. 
  \item A new algorithm for computing all the reducts in information systems which will be comparable to 
  		the state of the art algorithms in most datasets, and faster in some kind of datasets.
  \item A new algorithm for computing shortest reducts in information systems which will be comparable to 
  		the state of the art algorithms in most datasets, and faster in some kind of datasets.
  ...
  \item Software and hardware implementations for computing both, all the reducts and shortest reducts in 
  		information systems.
  \end{itemize}
...
\begin{enumerate}
	...
	\begin{itemize}
  		\item Generating a set of random datasets, systematically covering the space of possible combinations in 
  			  the values of the properties of basic matrices (factorial design).
  	...
  	\item Developing a new algorithm for computing all the reducts in information systems.
  	\begin{itemize}
  		\item Proposing a new algorithm for computing all the reducts based on relations found in 
  			  tasks~\ref{task1_all} and~\ref{task2_all}.
	...
	\begin{itemize}
  		...
  		\item Extracting the relevant subset of properties for determining a priori the appropriate 
  			  traversing strategy for a given dataset; and the rules governing this relation.
  		\item Proposing a new algorithm for computing shrotest reducts using the classifier found in
  			  the previous task.
  		\item Evaluating the proposed algorithm over synthetic and benchmarking datasets.
  	\end{itemize}
  	\item Finding a relationship between the traversed space and the expected cost of traversing strategies.
  	...
  	\begin{itemize}
  		\item Proposing a new algorithm for computing shortest reducts based on relations found in 
  			  tasks~\ref{task1_short} and~\ref{task2_short}.
  		\item Evaluating the proposed algorithm over synthetic and benchmarking datasets.
  	\end{itemize}
...
%  		\item Evaluate the implemented algorithm over synthetic and benchmarking datasets.
%  	\end{itemize}
  	Finally, the proposed algorithms will be redesigned and implemented in a hardware fashion in order to 
